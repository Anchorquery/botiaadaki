<template>
  <b-row>
    <b-colxx class="disable-text-selection">
      <list-page-heading
        title="Documentación - Temporadas"
        agregar="Temporada"
        :loadItems="loadItems"
    ...
    <b-colxx xxs="12">
      <b-modal
        id="modalbasic"
        ref="modalbasic"
        title="Eliminación de temporada"
      >
        <p>¿Está seguro que desea eliminar la temporada?</p>
        <b-alert show variant="danger">
          Todos los datos asignados a la temporada serán borrados.
        </b-alert>
    ...
    <template>
      <b-modal ref="modal-add" id="modal-add" centered title="Nueva temporada">
        <b-form>
          <b-container>
            <b-row>
              <b-col md="12">
  ...
  methods: {
    ...
    async crearItem() {
      try {
        ...
        let { data } = await axios.post("/temporadas", this.temporada, setting);
    ...
    async deleteItem() {
      ...
      try {
        ...
        let { data } = await axios.delete(`/temporadas/${this.categoriaRemove.id}`, setting);
